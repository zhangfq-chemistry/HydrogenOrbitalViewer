...
bool MolSymmetry::isRegularPolygon( vector < uint > idx)
{
    uint size0=idx.size();

    /*
    cout << "check it is "<< size0 <<endl;
    for (uint k = 0; k < size0; k++)
        cout << 1+ idx[k] <<" ";
    cout << endl;
*/
...
bool MolSymmetry::isRegularPolygon( vector < vector3 > coords)
{
    double eps=0.1;
...
bool MolSymmetry::isEquivalent (vector3  vec1, vector3 vec2)
{
...
bool MolSymmetry::isOrthogonal (vector3  vec1, vector3  vec2)
{
...
void MolSymmetry::setTolerance(double tole)
{
...
bool MolSymmetry::isInside (vector < vector3> & container, vector3 & v1)
{
    bool find=false;
...
MolSymmetry::MolSymmetry(double tolerance)
{
    ClearAll();
...
vector3 MolSymmetry:: FVector2Vector3 (FVector3 & vec)
{
    vector3 a(vec[0],vec[1],vec[2]);
    return a;
}

FVector3 MolSymmetry:: Vector2FVector3 ( vector3 & vec)
{
    FVector3 a(vec[0],vec[1],vec[2]);
    return a;
}
//tools functions end
//-----------------------------------------------------------------//
...
void MolSymmetry:: GetData()
{
    if (pAtoms==0 || pAtoms->size()<1){
        QMessageBox::information(0, "Warning","No Atoms is transfered to symmetry perception!");
        return;
    }
    ...
    for (uint i = 0; i < NAtoms; ++ i) {
        vector3 pos=FVector2Vector3((*pAtoms)[i].vPos);
        AtomCoordinates.push_back(pos);
        iElement = (*pAtoms)[i].AtomicNumber;
        AtomTypes.push_back(iElement);
        AtomMasses.push_back(GetAtomicMass(iElement, ATMASS_StandardAtomicWeight));
        AtomSymbols.push_back((*pAtoms)[i].ElementName());
    }
    ...
    cout << NAtoms <<endl;
    for (uint i = 0; i < NAtoms; ++ i) {
        cout <<AtomTypes[i] <<" " <<  AtomCoordinates[i]*0.529177 << "  "<<AtomMasses[i]<< endl;
    }
    */
}

void MolSymmetry:: SetData ()
{
    uint i;
    for (i = 0; i < NAtoms; ++ i) {
        FVector3 vec=Vector2FVector3(AtomCoordinates[i]);
        (*pAtoms)[i].setXYZ(vec);
    }
    ...
    DefaultBases[BASIS_Orbital] = "def2-TZVP";
    ...
    {
        FVector3 vPos=Vector2FVector3(AtomCoordinates[i+NAtoms]);
        FAtom   At(vPos, AtomSymbols[i+NAtoms], DefaultBases);
        At.AtomicNumber=AtomTypes[i+NAtoms];

        if(pGeometry) pGeometry->appendAtom(At);
    }

    NAtoms=NAtoms+addingAtoms;
    //append bonds
}



void MolSymmetry::ClearAll()
{
    addingAtoms=0;
    NAtoms=0;
...
void MolSymmetry::Translate(vector3 & vec)
{
    for (uint i = 0; i < NAtoms; ++ i) {
...
void  MolSymmetry::printInertialMoment()
{
    std::cout.setf(ios::fixed);
...
//mass-weighted
void  MolSymmetry::CalcInertialMoment()
{
...
MoleculeShape MolSymmetry::perceptMolShape()
{
    /*
...
void MolSymmetry:: buildDistanceMatrix()
{
    DistsMatrix.clear();
...
void MolSymmetry::buildSubGroups_perception()
{
    buildDistanceMatrix();
...
void MolSymmetry::buildSubGroups_perception_cluster()
{
    //unit sphere, plane
    vector < vector3 > S,P;
...
bool MolSymmetry::CheckMatrix( matrix3x3 & m)
{
/*
    cout << "\nCheck Matrix:\n";
...
bool MolSymmetry::CheckSigma(vector3  vec)
{
...
bool MolSymmetry::CheckS2n(vector3 vec, uint order)
{
...
bool MolSymmetry::CheckCn(vector3 vec, uint  order)
{
    if (vec.length() < 0.1) return false;
...
uint MolSymmetry::detectAxisOrder(vector3 vec)
{
    uint order0=1;
    uint maxOrder=1;
...
void MolSymmetry::detectPrincipalAxisOrder()
{
...
bool MolSymmetry::CombinationCn(vector < uint > & arr, uint data[],
...
void MolSymmetry::SearchCn(uint order0, vector < vector3 > & container)
{
    double eps = tol;
...
//----------------------------------------------------------
void MolSymmetry::SearchC5()
{
    SearchCn(5,C5);
}

void MolSymmetry::SearchC3()
{
    SearchCn(3,C3);
}

void MolSymmetry::SearchC4()
{
    SearchCn(4,C4);
}


void MolSymmetry::SearchC2()
{
    double eps = tol;
...
bool MolSymmetry::findFirstHorizontalC2(vector3 & firstOne)
{
    bool find=false;
...
bool MolSymmetry::perceptHorizontal_C2(bool isCheck=true)
{
...
void MolSymmetry::perceptVerticalMirror(vector < vector3 > & container)
{
    double eps = tol;
...
bool MolSymmetry::perceptMirrorH()
{
...
void MolSymmetry::perceptVerticalMirror(bool isCheck=true)
{
    //general cases
...
void MolSymmetry::appendVector3(vector < vector3 > & container, vector3 & vec)
{
    vector3 vec0=vec.normalize();
...
void MolSymmetry::refinePrincipalAxis ()
{
...
void MolSymmetry::PerceptSymmetry (bool isStandardOrientation)
{
...
void MolSymmetry::perceptOblate()
{
    isCi=false;
    isCs=false;
    isCn=false;
...
void MolSymmetry::perceptProlate()
{
    perceptOblate();
}


void MolSymmetry::perceptThMirror(bool isCheck=false)
{
    //has one center
    if (!hasI) {
...
void MolSymmetry::generalPerceptSymmetry()
{
    SearchC2();
...
void MolSymmetry::perceptTdMirror(bool isCheck=false)
{
    uint size = C3.size();
...
void MolSymmetry::perceptOhMirror(bool isCheck=false)
{
...
void MolSymmetry::perceptIhMirror(bool isCheck=false)
{
    uint size = C5.size();
    if ( size < 6) {
...
void MolSymmetry::perceptSphere()
{
...
void MolSymmetry::generate_I_from_2C2_orth(vector3 c2_1, vector3 c2_2)
{
    vector3 vec=cross(c2_1, c2_2);
    vector3 c2_3=vec.normalize();
...
void MolSymmetry::generate_I_from_2C5(vector3 c5_1, vector3 c5_2)
{
    C2.clear();
    C3.clear();
    C4.clear();
...
void MolSymmetry::generate_O_from_2C3(vector3 c3_1, vector3 c3_2)
{
    C2.clear();
    C3.clear();
    C4.clear();
...
void MolSymmetry::generate_O_from_2C4(vector3 c4_1, vector3 c4_2)
{
    C2.clear();
    C3.clear();
    C4.clear();
...
void MolSymmetry::generate_O_from_C4C2(vector3 c4, vector3 c2)
{
    C2.clear();
    C3.clear();
    C4.clear();
...
void MolSymmetry::report(string & pgsymbol)
{
...
void MolSymmetry::refineSymmetryElements()
{
...
void MolSymmetry::StandardOrientation()
{
...
void MolSymmetry::Orientation_Line()
{
    Orientation_Cn();
}

void MolSymmetry::Orientation_Ci()
{
    return;
}

void MolSymmetry::Orientation_Cs()
{
    vector3 vec0;
    if (SigmaH.size() > 0 )
        vec0=SigmaH[0];
...
matrix3x3 MolSymmetry::OrientationFromTo(vector3 Vec1, vector3 Vec2)
{
    matrix3x3 mat;
    mat.UnitMatrix();
...
void MolSymmetry::Orientation_Cn()
{
    rotMatrix = OrientationFromTo(PrincipalAxis,VZ);
}


void MolSymmetry::Orientation_S2n()
{
    Orientation_Cn();
}


void MolSymmetry::Orientation_Cnv()
{
...
void MolSymmetry::Orientation_Cnh()
{
...
void MolSymmetry::Orientation_Dn()
{
    vector3 c2;
    bool isfind=false;
    if (isEquivalent (PrincipalAxis,VZ)) { //check it has done or not?
...
void MolSymmetry::Orientation_Dnd()
{
    Orientation_Dn();
}

void MolSymmetry::Orientation_Dnh()
{
    Orientation_Dn();
}
...
void MolSymmetry::MolSymmetry::Orientation_T()
{
...
void MolSymmetry::Orientation_O()
{
    vector3 vec1=C4[0],vec2=C4[1];
    Orientation2ZX(vec1,vec2);
}

void MolSymmetry::Orientation_I()
{
    rotMatrix.UnitMatrix();
    rotMatrix2.UnitMatrix();
    rotMatrix1.UnitMatrix();
...
void MolSymmetry::Orientation2ZY(vector3 vec1, vector3 vec2)
{
...
void MolSymmetry::Orientation2ZX(vector3 vec1, vector3 vec2)
{
    double angle=vectorAngle(vec1,VZ);
    vector3 vec;
    if(angle > 1.0 && angle < 179.0 )
...
//--------------------------------------------
void MolSymmetry::buildOperation()
{
    PGOperation.clear();
...
void MolSymmetry::buildOperation_C00v()
{

}


void MolSymmetry::buildOperation_Cs()
{
    //E
...
//----------------------------------------------
void MolSymmetry::buildOperation_Ci()
{
    //E
    matrix3x3 matrix(1.0);
    PGOperation.push_back(matrix);
...
//---------------------------------------
void MolSymmetry::buildOperation_Cn(uint n)
{
    matrix3x3 matrix;
    uint i;
...
//---------------------------------------
void MolSymmetry::buildOperation_S2n(uint n)
{
    matrix3x3 matrix;
    int i;
...
//---------------------------------------
void MolSymmetry::buildOperation_Cnv(uint n)
{
    matrix3x3 matrix;
...
//---------------------------------------
void MolSymmetry::buildOperation_Cnh(uint n)
{
    matrix3x3 matrix;
    angle=360.0/double(n);
    double ang=0.0;
...
//---------------------------------------
void MolSymmetry::buildOperation_Dn(uint n)
{
    matrix3x3 matrix;
    uint i;
    angle=360.0/double(n);
...
//--------------------------------------------------
void MolSymmetry::buildOperation_Dnd(uint n)
{
    matrix3x3 matrix;
    int i;
    angle=360.0/double(n);
...
//--------------------------------------------
void MolSymmetry::buildOperation_Dnh(uint n)
{
    matrix3x3 matrix;
    int i;
    angle=360.0/double(n);
...
//-----------------------------------------------------------
void MolSymmetry::buildOperation_T()
{
    matrix3x3 matrix,C2;
    int i;
    double ang=0.0;
...
//-------------------------------------------------------
void MolSymmetry::buildOperation_Th()
{
    //!Th=T*Ci;
    int i=0;
    buildOperation_T();
...
//-------------------------------------------------------
void MolSymmetry::buildOperation_Td()
{
   buildOperation_T();
...
//--------------------------------------------
void MolSymmetry::buildOperation_O()
{
    matrix3x3 matrix;
    int i,j;
    double ang=0.0;
...
//--------------------------------------------
void MolSymmetry::buildOperation_Oh()
{
    //Oh=O*Ci;
    buildOperation_O();
...
//--------------------------------------------
void MolSymmetry::buildOperation_I()
{
    matrix3x3 matrix;
    unsigned int i,j;
    double ang,ang1;
...
//--------------------------------------------
void MolSymmetry::buildOperation_Ih()
{
    //!Ih=I*Ci;
    buildOperation_I();
...
void MolSymmetry::buildInvOperation()
{
    InvPGOperation.clear();
...
void MolSymmetry::buildOperation_D00h()
{
    //E
    matrix3x3  matrix;
    matrix.Set(0,0,1.0);
...
void MolSymmetry::setSymmetry(string pgsybol)
{
    PGSymbol = pgsybol;
...
void MolSymmetry::ParseSymmetrySybmol (string schoefiles)
{
    ClearAll();
...
bool MolSymmetry::patchMolecule(uint & NAtomsofPatched)
{
    ParseSymmetrySybmol(PGSymbol);
...
bool MolSymmetry::buildSubGroupsOnSymmetry_patch()
{
...
bool MolSymmetry::buildSubGroupsOnSymmetry_refine()
{
    buildOperation ();
...
void  MolSymmetry::patchC00v()
{
    cout<< "Cannot patch based on C00v symmetry!\n";
    return;
}

void  MolSymmetry::patchD00h()
{
    addingAtoms=0;
    vector3 vec1, vec2;
