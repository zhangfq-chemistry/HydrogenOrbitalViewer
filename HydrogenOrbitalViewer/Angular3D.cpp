...
//theta node
void Angular3D::renderThetaNodes(HMol * mol)
{
    ...
    for (uint i =0; i<mol->NOrbitals();i++) //each orbital
    {
         ...
         for (int k=0;k<mol->getObtial(i)->getAO(j)->Nodes_T.size();k++)
         {
             ...
             length=mol->getObtial(i)->getLengthYlm();
             height=mol->getObtial(i)->getHeightYlm();
...
void Angular3D::renderCones(double height, double cos2angle)
{
    ...
    parametricObject->SetCrossSectionRadius(0.02);
...
void Angular3D::renderAllNodes(HMol * mol)
{
    renderPhiNodes(mol);
...
void Angular3D::renderPhiNodes(HMol * mol)
{
    double thikness=0.02;
    double length=3.0,height=3.0;
    ...
    for (uint i =0; i<mol->NOrbitals();i++) //each orbital
    {
         ...
         if (mol->getObtial(i)->isMolecularOrbital) continue;
           ...
           if (!mol->getObtial(i)->isVisible_angular_node()) continue;
           if (mol->getObtial(i)->getAO(0)->Nodes_P.size()<1) continue;



           // only one AO inside each MO
           uint nNodes=mol->getObtial(i)->getAO(0)->Nodes_P.size();

           //cout <<"Number of Phi Nodes: " << nNodes<<endl;


           for (int k=0; k<nNodes; k++)
           {
               double angle=mol->getObtial(i)->getAO(0)->Nodes_P[k];
               ...
               if(! mol->getObtial(i)->getAO(0)->isXOYNode )
                   continue;
...
void Angular3D::renderMol(HMol * mol)
{
    removeAllActors();
    renderOrbital(mol);
...
void Angular3D::renderOrbital(HMol * mol)
{
    ...
    for (uint k =0; k<NMOs;k++) //each orbital
    {
        ...
        {
            if (mol->getObtial(k)->isPosYlmExisted())
            {
                lineWidth=orbitalLineWidth;
                ...
                reverse->SetInputData(mol->getObtial(k)->yData);
        ...
        if (mol->getObtial(k)->isNegLobeVisible())
        {
            if (mol->getObtial(k)->isNegYlmExisted())
            {
                lineWidth=orbitalLineWidth;
                ...
                auto reverse = vtkSmartPointer<vtkReverseSense>::New();
                reverse->SetInputData(mol->getObtial(k)->yData1);
                reverse->ReverseCellsOn();
                reverse->ReverseNormalsOn();

                mapper_neg->SetInputConnection(reverse->GetOutputPort());
